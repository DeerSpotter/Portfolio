...
try {
  ...
  await run('Syntax: theme browser proof', process.execPath, ['--check', 'verify-theme-browser.mjs']);
  ...
  await run('Asteroid traffic motion contract', process.execPath, ['verify-asteroid-traffic.mjs']);
  ...
  await run('Military dark theme toggle', process.execPath, ['verify-theme-browser.mjs']);
