...
async function exercise(viewport, label, reducedMotion = false) {
  ...
  const samples = await page.evaluate(async () => {
    ...
    for (let frame = 0; frame < 180; frame++) {
      ...
      if (!ship?.ready || !canvas?.ready) continue;
      result.push({
  ...
  for (let index = 1; index < samples.length; index++) {
    ...
    if (Math.abs(current.targetRoll) > 0.04
      && Math.abs(current.turnSignal) > 0.001
  ...
  const seam = await page.evaluate(async () => {
    ...
    for (let frame = 0; frame < 120; frame++) {
      ...
      capture('release');
      const ship = window.__portfolioShipDebug;
      if (!ship.bank.seamNeutralizing
        && ship.bank.seamDistance >= 0.04
        && ship.camera.seamBlend < 0.02) break;
    }
  ...
  const preWrapSettled = [...approachFrames].reverse().find(sample =>
    sample.loopCycle === seam.cycleBefore && sample.progress >= 0.97
  );
  ...
  const seamFrames = seam.result.filter(sample => sample.phase === 'wrap-transition' || sample.phase === 'seam');
  ...
  for (let index = 0; index < seamFrames.length; index++) {
    const current = seamFrames[index];
    maxSeamCameraBankOffset = Math.max(maxSeamCameraBankOffset, Math.abs(current.cameraBankOffset));
    if (index > 0 && current.sampleIndex === seamFrames[index - 1].sampleIndex + 1) {
      maxSeamRollStep = Math.max(maxSeamRollStep, Math.abs(current.roll - seamFrames[index - 1].roll));
    }
  }
  ...
  for (let index = 0; index < visualSeamFrames.length; index++) {
    ...
    if (index > 0) {
      const previous = visualSeamFrames[index - 1];
      if (current.sampleIndex === previous.sampleIndex + 1) {
        contiguousScreenPairs += 1;
        maxScreenStep = Math.max(maxScreenStep, Math.abs(current.screenX - previous.screenX));
      }
    }
  }

  if (contiguousScreenPairs < 10) {
    throw new Error(`${label}: insufficient contiguous screen-space seam pairs: ${contiguousScreenPairs}`);
  }
  ...
  const seamSigns = new Set(
    neutralFrames.filter(sample => Math.abs(sample.roll) > 0.02).map(sample => Math.sign(sample.roll)),
  );
...
try {
  ...
  console.log('[portfolio-ship-bank] visual=continuous-frame-screen-space-seam-regression');
