...
async function exercise(viewport, label, reducedMotion = false) {
  ...
  // Reproduce the real document reloop. A cycle change is transport across the
  // scroll seam, not a new turn. While that seam is active, roll must target
  // zero only; it may not kick right and then left before normal banking resumes.
  const seam = await page.evaluate(async () => {
    ...
    // Continue into the next cycle so the test also proves the neutral hold is
    // released only after the ship is level and clear of the seam.
    scrollTo(0, max * 0.08);
